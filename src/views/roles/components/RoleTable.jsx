...
const RoleTable = ({ roles = [], onEditRole, onDeleteRole }) => {
    ...
    const columns = [
        ...
        { title: 'Módulos permitidos', data: 'modules_list' },
    ...
    const data = roles.map((role) => ({
        ...
        modules_list: role.modules && role.modules.length > 0
            ? role.modules.map(m => m.name).join(', ')
            : '—',
        created_at: new Date(role.created_at).toLocaleDateString('es'),
        id: role.id,
    }));
    ...
    const options = {
        ...
        createdRow: (row, data) => {
            ...
            if (editBtn) editBtn.addEventListener('click', () => onEditRole(data));
            if (deleteBtn) deleteBtn.addEventListener('click', () => onDeleteRole(data));
        },
