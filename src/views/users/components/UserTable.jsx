...
const UsersTable = ({ users = [], onEditUser, onDeleteUser }) => {
    ...
    const data = users.map((user) => ({
        fullname: `${user.person?.name || ''} ${user.person?.last_name || ''}`,
        username: user.username,
        role: user.role?.name || '—',
        created_at: new Date(user.created_at).toLocaleDateString('es'),
        id: user.id,
    ...
    const options = {
        ...
        createdRow: (row, data) => {
            ...
            if (editBtn) editBtn.addEventListener('click', () => onEditUser(data));
            if (deleteBtn) deleteBtn.addEventListener('click', () => onDeleteUser(data));
        },
