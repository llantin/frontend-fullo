...
const ContactList = ({
  ...
  const filteredContacts = contacts.filter(contact => contact.name.toLowerCase().includes(searchTerm.toLowerCase()) || (contact.lastMessage?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false));
  return <Card className="h-100 mb-0 border-end-0 rounded-end-0">
      <CardHeader className="p-3 border-light card-bg d-block">
        <div className="d-flex gap-2">
          <div className="app-search flex-grow-1">
            <FormControl type="text" className="bg-light-subtle border-light" placeholder="Search here..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      height: 'calc(100% - 100px)'
        ...
        <ListGroup variant="flush" className="chat-list">
          {filteredContacts.map(contact => <ListGroupItem key={contact.id} action className={`d-flex gap-2 justify-content-between ${contact.id === currentContact.id ? 'active' : ''}`} onClick={() => {
          setContact(contact);
