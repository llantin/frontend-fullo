...
const ProductReviews = () => {
  const columns = [columnHelper.accessor('userName', {
    header: 'Reviewer',
    cell: ({
      row
    }) => <div className="d-flex justify-content-start align-items-center gap-2">
                    <div className="avatar avatar-sm">
                        <img src={row.original.userAvatar} alt={`avatar-${row.original.id}`} width={32} height={32} className="img-fluid rounded-circle" />
                    </div>
                    <div>
    ...
    enableSorting: false,
    header: 'Actions',
    cell: () => <div className="d-flex justify-content-center gap-1">
  ...
  const [data, _setData] = useState(() => [...reviewsData]);
  ...
  return <Card className="mt-5 shadow-none border border-dashed">
            ...
            <div className="table-responsive">
                ...
                {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
                        <TablePagination totalItems={totalItems} start={start} end={end} showInfo itemsName="reviews" previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
                    </CardFooter>}
            </div>
        </Card>;
};
